...
const DeferUntilConnected = (superClass) =>
...
const QuickDrawElement = DeferUntilConnected(
  class extends LitElement {
    ...
    constructor() {
      super();
      this.category = null;
      this.key = "AIzaSyC0U3yLy_m6u7aOMi9YJL2w1vWG4oI5mj0";
      this.index = -1;
      this.x = "inherit";
      this.y = "inherit";
      this.width = "auto";
    ...
    updated(changedProperties) {
      ...
      if (this._isFetching) {
        console.warn(
          "Tried to change property while fetch is still in flight."
        );
        return;
      }
    ...
    async fetchImageData(category, index) {
      let filepath =
        this.host +
        `/drawing/${category}?isAnimated=${this.animated}&key=${this.key}`;
    ...
    async drawImage() {
      ...
      this.currentImageScaleRatio =
        Math.floor(this.currentImageScaleRatio * 0.9 * 100) / 100;

      // need to get time ratios for each path, used when user sets time
      // first get total time for drawing
      if (this.animated) {
        let total =
          imageData[imageData.length - 1][2][
            imageData[imageData.length - 1][2].length - 1
          ];
      ...
      for (let i = 0; i < imageData.length; i++) {
        if (this.animated) {
          if (this.time) {
            ...
            timeToCompleteThisPath =
              imageData[i][2][imageData[i][2].length - 1];
          }
          if (i > 0) {
            if (this.time) {
              timeToCompleteLastPath = imageData[i - 1][3] * this.time;
            } else {
              timeToCompleteLastPath =
                imageData[i - 1][2][imageData[i - 1][2].length - 1];
            }
            timeToCompleteThisPath -= timeToCompleteLastPath;
          }
        }
        this._log(
          `Path ${i + 1}/${imageData.length} time: ${timeToCompleteThisPath}ms`,
          "#e2b537"
        );
        try {
          await this._drawSegment(imageData[i], timeToCompleteThisPath);
        } catch (err) {
          throw new Error("Segment drawing cut short.");
        }
        this.promisePathCancel = null;
        this.lastIndex = 0;
      }
      this.dispatchEvent(
        new CustomEvent("drawingComplete", {
          detail: {
    ...
    _transformPoint(x, y) {
      x =
        (x - this.currentImageProperties.x) * this.currentImageScaleRatio +
        this.strokeWidth;
      y =
        (y - this.currentImageProperties.y) * this.currentImageScaleRatio +
        this.strokeWidth;
      return { x: x, y: y };
    }
